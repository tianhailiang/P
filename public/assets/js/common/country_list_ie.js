...
function screenTag (country) {
  ...
  if (country == 0 && eduList[0] == 0) {
    //默认国家全部 学历全部 循环所有标签
    console.log('默认国家全部 学历全部 循环所有标签')
    for (let eItem =0;eItem < edu_list.length;eItem++) {
...
function checkEdu () {
  ...
  $('.level-sel').on('click',function(){
    ...
    if($(this).attr('checked')){
      ...
      eduId_list.findIndex(function(value, index, arr) {
        if(value == Number($(that).index()+2)) {
          arr.splice(index,1);
        }
      })
      console.log('去掉勾选', eduId_list)
    }else{
      // console.log('true')
      $(this).find('.level-sel-i').css({'border':'none','color':'#c13232','marginRight':'7px','fontSize':'16px'}).html('&#xe640;');
      $(this).find('.level-sel-i').next().css({'color':'#c13232'});
    ...
    eduId_list.unshift(Number($('#apply-country').val()))
    console.log('勾选学历集合',eduId_list)
