...
function showcity(){
  if($.cookie('currentarea')){
    ...
    $(".city").html(iDcity1[$.cookie('currentarea')][0]);
...
//顶部搜索
$("#searchNewsBtn2").click(function() {
  var so_type = '资讯';
  var so_url = 'so_news';
  var so_key_word = $.trim($("#searchNews2").val());
  if (so_key_word.length == 0) {
    $('#searchNews2').focus();
  }
  else {
    $.cookie('searchType', so_type);
    $.cookie('keyWord', so_key_word);

    window.open(fn.no_urlgen(so_url, 'q=' + so_key_word));
  }
})
$(".nav-eighth .will").hover(function(){
  $(this).find(".will-do").slideDown(100);
  $(this).find(".go-down").html("&#xe661;");
},function(){
  $(this).find(".will-do").slideUp(100);
  $(this).find(".go-down").html("&#xe60b;")
});
